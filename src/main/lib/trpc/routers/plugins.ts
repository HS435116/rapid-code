...
export const pluginsRouter = router({
  ...
  /**
   * Browse plugins from GitHub (search by topic:claude-plugin)
   */
  browseGitHub: publicProcedure
    .input(
      z.object({
    ...
    .query(async ({ input }) => {
      const plugins = await searchGitHubPlugins(
        input?.query,
        input?.forceRefresh,
      )
      return plugins
  ...
  /**
   * Uninstall a plugin by its source identifier
   */
  uninstallPlugin: publicProcedure
    ...
    .mutation(async ({ input }) => {
      return uninstallPlugin(input.source)
    }),
})
