      ...
      return observable<UIMessageChunk>((emit) => {
        ...
        if (existingController) {
          existingController.abort()
	        ...
	        // Connection timeout: abort SDK query if no response within 60s
	        // Prevents hanging when proxy/relay is unreachable or slow to respond
	        const connectionTimeout = setTimeout(() => {
	          if (!abortController.signal.aborted) {
	            console.log(`[SD] M:TIMEOUT sub=${subId} - SDK connection timed out after 60s`)
	            abortController.abort(new Error('Connection timed out after 60s'))
	          }
	        }, 60000)
        ...
        ;(async () => {
          try {
            ...
            // Security: filter out invalid custom model names before using config
            // Prevents errors from third-party/misconfigured models
            const rawCustomConfig = offlineResult.config || input.customConfig
            if (rawCustomConfig?.model) {
              const modelLower = rawCustomConfig.model.toLowerCase()
              const isClaudeModel =
              ...
              if (!isClaudeModel) {
                console.warn(
                  `[Claude] Custom model "${rawCustomConfig.model}" is not a default Claude model. Make sure your API provider supports this model.`,
                )
              }
            }
