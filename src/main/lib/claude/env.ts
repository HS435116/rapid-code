...
export function getClaudeShellEnvironment(): Record<string, string> {
  ...
  if (isWindows()) {
    ...
    // Merge important process.env vars (including .env file values like ANTHROPIC_AUTH_TOKEN)
    // This ensures CLI config detection picks up keys from the app's .env file
