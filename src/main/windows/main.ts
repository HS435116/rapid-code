...
function registerIpcHandlers(): void {
  ...
  ipcMain.handle("app:set-badge", (event, count: number | null) => {
    ...
    if (process.platform === "darwin") {
      ...
      if (count !== null && count > 0) {
        win.setTitle(`Rapid Code (${count})`)
      } else {
        win.setTitle("Rapid Code")
  ...
  const validateSender = (event: Electron.IpcMainInvokeEvent): boolean => {
    ...
    try {
      ...
      const trusted = ["21st.dev", "localhost", "127.0.0.1", "0.0.0.0"]
  ...
  ipcMain.handle(
    ...
    async (
      ...
      const isLocalDev = token === "bypass-token-local-dev"

      // Local dev mode: return mock data without making HTTP requests
      if (isLocalDev) {
        if (url.includes("teams.getUserTeams")) {
          return { ok: true, status: 200, data: [], error: null }
        }
        if (url.includes("user/plan")) {
          return { ok: true, status: 200, data: { plan: "free" }, error: null }
        }
      ...
      try {
        const response = await fetch(url, {
          ...
          headers: {
            ...options?.headers,
            ...(isLocalDev ? {} : { "X-Desktop-Token": token }),
...
export function createWindow(options?: { chatId?: string; subChatId?: string }): BrowserWindow {
  ...
  // Inject translation engine (中文翻译壳)
  injectTranslator(window)
