...
export class AuthManager {
  ...
  /**
   * Get API base URL from shared config (supports MAIN_VITE_API_URL override)
   */
  ...
  async updateUser(updates: { name?: string }): Promise<AuthUser | null> {
    ...
    try {
      ...
      if (token) {
        ...
        if (!response.ok) {
          console.warn("[Auth] Server update failed, updating locally only")
        }
      }
    } catch (err) {
      console.warn("[Auth] Server update error, updating locally only:", err)
    }

    // Update locally (always succeeds)
  ...
  async fetchUserPlan(): Promise<{ email: string; plan: string; status: string | null } | null> {
    ...
    // Local dev: return mock plan
    if (token === "bypass-token-local-dev") {
      return { email: "dev@local.dev", plan: "free", status: "active" }
    }
