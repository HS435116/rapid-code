...
try {
  // In dev mode, __dirname = out/main/
  // Project root = ../../ (.env is at D:\onecode\.env)
  const possiblePaths = [
    join(__dirname, "../../.env"),           // dev mode
    join(__dirname, "../.env"),              // fallback
    join(process.cwd(), ".env"),             // cwd fallback
  ]
  for (const envFile of possiblePaths) {
    if (existsSync(envFile)) {
      ...
      console.log("[App] Loaded .env from", envFile)
      break
    }
  }
} catch (err) {
...
<head>
  ...
  <title>Rapid Code - Authentication</title>
  <style>
...
if (gotTheLock) {
  ...
  app.whenReady().then(async () => {
    ...
    app.setAboutPanelOptions({
      applicationName: "晨曦微光工作室",
      applicationVersion: app.getVersion(),
      version: `Claude Code ${claudeCodeVersion}`,
      copyright: "Copyright © 2026 晨曦微光工作室",
    })
          ...
          submenu: [
            ...
            { type: "separator" },
            {
              label: isCliInstalled()
                ? "Uninstall 'rapidcode' Command..."
                : "Install 'rapidcode' Command in PATH...",
                ...
                if (isCliInstalled()) {
                  ...
                  if (result.success) {
                    dialog.showMessageBox({
                      ...
                      detail: "The 'rapidcode' command has been removed from your PATH.",
                    })
                    buildMenu()
                  } else {
                  ...
                  if (result.success) {
                    dialog.showMessageBox({
                      ...
                      detail:
                        "You can now use 'rapidcode .' in any terminal to open Rapid Code in that directory.",
                    })
                    buildMenu()
                  } else {
    ...
    if (app.isPackaged && !process.env.DISABLE_UPDATES) {
    ...
    } else if (app.isPackaged && process.env.DISABLE_UPDATES) {
      console.log("[App] Auto-updates disabled via DISABLE_UPDATES env var")
    }
    ...
    // Handle directory argument from CLI (e.g., `rapidcode /path/to/project`)
    parseLaunchDirectory()
