...
export type ModelProfile = {
  ...
  rawJson?: string // Raw JSON string as entered by user
  requestTemplate?: string // User-defined request JSON template with {user_input}
}
...
// Master toggle for custom models (profiles show in chat selector)
export const customModelsEnabledAtom = atomWithStorage<boolean>(
  "agents:custom-models-enabled",
  true, // Enabled by default
  undefined,
  { getOnInit: true },
)
...
export const activeConfigAtom = atom((get) => {
  // If custom models disabled, never use custom config
  if (!get(customModelsEnabledAtom)) return undefined
  ...
  // Fallback: check legacy single config (customClaudeConfigAtom) for backward compatibility
  const legacyConfig = get(customClaudeConfigAtom)
  if (legacyConfig?.model && legacyConfig?.token && legacyConfig?.baseUrl) {
    return legacyConfig
  }

  // No custom config - use default (Claude Code will use its own defaults)
  return undefined
})
