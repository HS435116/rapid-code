...
export function AgentsLayout() {
  ...
  // Add timeout to prevent app from hanging if tRPC IPC fails
  ...
  useEffect(() => {
    ...
    projectsTimeoutRef.current = setTimeout(() => {
      console.warn("[AgentsLayout] Projects list timed out, proceeding without validation")
  ...
  // Also skip while projects are loading or if query timed out
  useEffect(() => {
    if (!projects && !projectsTimedOut) return // Don't change sidebar state while loading and not timed out
    if (isLoadingProjects && !projectsTimedOut) return // Wait for loading to complete unless timed out
        ...
        >
          ...
          {!isMobile && !sidebarOpen && (
            <button
              ...
              className="absolute left-0 top-1/2 -translate-y-1/2 z-40 w-5 h-12 flex items-center justify-center rounded-r-md bg-background border border-l-0 border-border text-muted-foreground hover:text-foreground hover:bg-accent/50 transition-colors cursor-pointer"
