...
interface AgentModelSelectorProps {
  ...
  claude: {
    ...
    customModels?: { id: string; name: string; model: string }[]
...
type FlatModelItem =
  ...
  | { type: "custom-profile"; profileId: string; profileName: string; modelName: string }
...
export function AgentModelSelector({
  ...
  const allModels = useMemo<FlatModelItem[]>(() => {
    ...
    // Add custom model profiles as selectable items
    if (claude.hasCustomModelConfig && claude.customModels) {
      for (const profile of claude.customModels) {
        items.push({
          type: "custom-profile",
  ...
  const filteredModels = useMemo(() => {
    ...
    return allModels.filter((item) => {
      switch (item.type) {
        ...
        case "custom-profile":
          return item.profileName.toLowerCase().includes(q) || item.modelName.toLowerCase().includes(q)
      }
    })
  }, [allModels, search])
  ...
  const isItemSelected = (item: FlatModelItem): boolean => {
    switch (item.type) {
      ...
      case "custom-profile":
        return selectedAgentId === "claude-code" && claude.activeCustomProfileId === item.profileId
    }
  }
  ...
  const handleItemClick = (item: FlatModelItem) => {
    ...
    switch (item.type) {
      ...
      case "custom-profile":
        if (!canSelectProvider("claude-code")) return
        onSelectedAgentIdChange("claude-code")
        claude.onSelectCustomProfile?.(item.profileId)
        break
    }
    handleOpenChange(false)
  }
  ...
  const getItemIcon = (item: FlatModelItem) => {
    switch (item.type) {
      ...
      case "custom":
        return <ClaudeCodeIcon className="h-3.5 w-3.5 text-muted-foreground shrink-0" />
      case "custom-profile":
        return <ClaudeCodeIcon className="h-3.5 w-3.5 text-foreground shrink-0" />
    }
  }
