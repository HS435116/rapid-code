...
export async function toDraftFile(
  ...
  try {
    // Use existing base64 if available (from initial upload), otherwise fetch blob URL
    const base64Data = file.base64Data || await blobUrlToBase64(file.url)
    return {
      id: file.id,
...
export function fromDraftFile(draft: DraftFile): UploadedFile | null {
  ...
  try {
    ...
    return {
      ...
      filename: draft.filename,
      url,
      base64Data: draft.base64Data, // Preserve base64 for API transmission
      size: draft.size,
      type: draft.type,
      isLoading: false,
    }
