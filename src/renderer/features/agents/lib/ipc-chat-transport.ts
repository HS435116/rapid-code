...
export class IPCChatTransport implements ChatTransport<UIMessage> {
  ...
  async sendMessages(options: {
    ...
    const storedCustomConfig = appStore.get(
      activeConfigAtom,
    ) as CustomClaudeConfig | undefined
    // activeConfigAtom already handles toggle + profile selection logic
    // Only send customConfig when a valid custom config is active
    const customConfig = normalizeCustomClaudeConfig(storedCustomConfig ?? { model: "", token: "", baseUrl: "" })
    ...
    return new ReadableStream({
      start: (controller) => {
        // Check if we should use OpenAI direct endpoint
        // Routing: non-Anthropic baseUrl → openai.chat (OpenAI-compatible API via proxy)
        // Anthropic baseUrl → claude.chat (Claude Code SDK)
        const isOpenAIModel = customConfig && (
          !customConfig.baseUrl?.toLowerCase().includes("anthropic.com")
        )
        console.log(`[SD] Route: ${isOpenAIModel ? "openai.chat" : "claude.chat"} (model: ${customConfig?.model || "none"}, baseUrl: ${customConfig?.baseUrl || "none"})`)

        const chatEndpoint = isOpenAIModel
          ? (trpcClient as any).openai.chat.subscribe
          : trpcClient.claude.chat.subscribe

        const chatInput = isOpenAIModel
          ? {
              prompt,
              ...
              cwd: this.config.cwd,
              images,
              // Pass full message history for conversation memory
              messages: options.messages.map((m) => ({
                role: m.role as "user" | "assistant" | "system",
                content: this.extractText(m),
              })),
              requestTemplate: (() => {
                const allProfiles = appStore.get(modelProfilesAtom) as any[]
                const activeId = appStore.get(activeProfileIdAtom)
                const activeProfile = allProfiles.find(p => p.id === activeId)
                return activeProfile?.requestTemplate || undefined
              })(),
            }
          : {
        ...
        const sub = chatEndpoint(chatInput, {
    ...
    if (msg.parts) {
      ...
      for (const p of msg.parts) {
        ...
        if (partType === "text" && (p as any).text) {
          ...
          fileContents.push(`\n--- ${fileName} ---\n${fc.content}`)
        } else if (partType === "data-file") {
          // File attachment with base64 data - add file info to prompt
          const data = (p as any).data
          if (data && data.filename && data.base64Data) {
            // Only decode text-based files inline (code, docs, etc.)
            const textExtensions = new Set([
              ...
              ".svg", ".env", ".gitignore", ".dockerfile", ".editorconfig",
            ])
            const ext = data.filename.toLowerCase().slice(data.filename.lastIndexOf("."))
            if (textExtensions.has(ext)) {
              try {
                const decoded = atob(data.base64Data)
                fileContents.push(`\n--- Attachment: ${data.filename} ---\n${decoded}`)
              } catch {
                fileContents.push(`\n[Attachment: ${data.filename} - unable to decode]`)
              }
            } else {
              fileContents.push(`\n[Attachment: ${data.filename} (${data.size || "unknown size"} bytes)]`)
            }
          }
        }
      }
