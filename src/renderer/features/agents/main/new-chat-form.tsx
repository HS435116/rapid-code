...
export function NewChatForm({
  ...
  const activeCustomConfig = useAtomValue(activeConfigAtom)
  const normalizedCustomClaudeConfig =
    normalizeCustomClaudeConfig(activeCustomConfig ?? { model: "", token: "", baseUrl: "" })
  ...
  const customProfileModels = useMemo(() => {
    return profiles.filter(p => !p.isOffline).map(p => ({ id: p.id, name: p.name, model: p.config.model }))
  }, [profiles])
  ...
  const selectedModelLabel = useMemo(() => {
    ...
    // Show active custom profile name
    if (activeProfileId) {
...
/**
 * Generate a concise, readable chat title from the user's input message.
 * Instead of raw character slicing, this extracts the core intent:
 * - Strips slash commands, mention syntax, and system directives
 * - Finds the key action noun phrase (first ~3-5 meaningful words)
 * - Falls back to trimmed first line if no pattern matches
 */
function generateChatTitle(rawMessage: string): string {
  const message = rawMessage.trim()
  if (!message) return "New Chat"

  // Strip slash commands and their arguments
  let cleaned = message.replace(/^\/\w+(?:\s+\S+)*\s*/g, "").trim()

  // Strip @mentions (e.g., @[file:...] or @agent:...)
  cleaned = cleaned.replace(/@\[[^\]]*\]/g, "").trim()

  // Strip quoted blocks that look like system instructions
  cleaned = cleaned.replace(/`[^`]*`/g, "").trim()

  // Try to extract the first sentence or clause (up to period, newline, or colon)
  const sentenceMatch = cleaned.match(/^([^.!\n:]{3,80})(?:[.!\n:]|$)/)
  if (sentenceMatch) {
    const sentence = sentenceMatch[1].trim()
    // Take the first 3-6 meaningful words
    const words = sentence.split(/\s+/).filter(w => w.length > 0)
    if (words.length <= 6) {
      return sentence.charAt(0).toUpperCase() + sentence.slice(1)
    }
    // Truncate to ~6 words for readability
    const truncated = words.slice(0, 6).join(" ")
    return truncated.charAt(0).toUpperCase() + truncated.slice(1)
  }

  // Fallback: first line, trimmed and capitalized, max 60 chars
  ...
  return firstLine.slice(0, 57) + '...'
    ...
    if (!selectedProject) {
      // If no project selected, show a toast hint instead of opening a folder picker
  ...
  createChatMutation.mutate({
      ...
      name: generateChatTitle(message.trim()),
                      ...
                      <div className="group/model-controls flex items-center gap-0.5">
                        <AgentModelSelector
                          ...
                          claude={{
                            ...
                            customModels: customProfileModels,
                            activeCustomProfileId: activeProfileId,
                            onSelectCustomProfile: (profileId) => setActiveProfileId(profileId),
