  ...
  const handleForkFromMessage = useCallback(
    async (messageId: string) => {
      ...
      try {
        ...
        // Auto-recovery: if messages weren't persisted to DB yet, save them and retry once
        if (
          !forkRetried &&
          ...
          try {
            ...
            if (Array.isArray(currentMessages) && currentMessages.length > 0) {
              await trpcClient.chats.updateSubChatMessages.mutate({
                id: subChatId,
                messages: JSON.stringify(currentMessages),
              })
            }
          } catch {
            // If persistence also fails, fall through to error handler
          }

          // Retry once with persisted messages
          const retryIndex = messagesForForkRef.current.findIndex(m => m.id === messageId)
          if (retryIndex !== -1) {
            try {
              const retryResult = await trpcClient.chats.forkSubChat.mutate({
                subChatId,
                messageId,
                messageIndex: retryIndex,
              })

              const newSubChat = retryResult.subChat
              const newMode = (newSubChat.mode as "plan" | "agent") || "agent"
      ...
      ...currentFiles
        ...
        .map((f) => ({
          ...
          data: {
            ...
            filename: f.filename,
            size: f.size,
            base64Data: f.base64Data, // Include base64 data for API transmission
          },
        })),
    ]
    ...
    (subChatId: string): Chat<any> | null => {
      ...
      const newChat = new Chat<any>({
        ...
        onFinish: () => {
          ...
          // Persist messages to DB so fork/rollback work reliably
          // IMPORTANT: AI SDK manages messages in-memory but OpenAI/streaming
          // routes don't save to DB automatically. Without this, forkSubChat
          // can't find messages and throws "Message not found".
          try {
            const currentMessages = (newChat as any)?.messages
            if (Array.isArray(currentMessages) && currentMessages.length > 0) {
            ...
            // Best-effort persistence - fork will still work if messages
            // are persisted before the fork attempt
          }
